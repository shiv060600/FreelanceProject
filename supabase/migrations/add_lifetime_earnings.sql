...
-- Add lifetime client earnings columns safely (idempotent)
ALTER TABLE public.clients
ADD COLUMN IF NOT EXISTS lifetime_client_earnings FLOAT DEFAULT 0,
ADD COLUMN IF NOT EXISTS lifetime_client_paid_invoices INT DEFAULT 0;

-- Create function to update client lifetime stats
CREATE OR REPLACE FUNCTION update_lifetime_client_stats()
RETURNS TRIGGER AS $$
BEGIN
...
-- Drop and recreate client stats trigger
...
CREATE TRIGGER trigger_update_lifetime_client_stats
  ...
  FOR EACH ROW
  EXECUTE FUNCTION update_lifetime_client_stats();
...
-- Add comments to document the correct business logic
